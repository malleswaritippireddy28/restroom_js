        ...
        var S = function (y) {
          ...
          if (a === "canvas" && y === "rr_dataURL") {
            ...
            k.currentSrc.startsWith("data:") ||
              (k.setAttribute("falconWeb-original-src", t.attributes.src),
              (k.src = g.toString()));
          }
      ...
      return (
        typeof o == "function" &&
          ...
          Object.defineProperties(o, {
            __falconWeb_original__: { enumerable: !1, value: r },
          })),
        (t[e] = o),
        () => {
          t[e] = r;
  ...
  function be(t = {}) {
    ...
    V = (I, oe) => {
      ...
      if (
        ...
        const B = { type: "falconWeb", event: te(I), isCheckout: oe };
    ...
    play(e = 0) {
      ...
      this.service.state.matches("paused")
        ...
        (r =
          ...
          r.classList.remove("falconWeb-paused"),
      ...
      e === void 0 &&
        ...
        (r =
          (n = this.iframe.contentDocument) == null
            ? void 0
            : n.getElementsByTagName("html")[0]) == null ||
          r.classList.add("falconWeb-paused"),
      ...
      this.insertStyleRules(a, s),
        this.service.state.matches("playing") ||
          this.iframe.contentDocument
            ...
            .classList.add("falconWeb-paused"),
    ...
    insertStyleRules(e, n) {
      ...
      if (
        (this.config.pauseAnimation &&
          o.push(
            "html.falconWeb-paused *, html.falconWeb-paused *:before, html.falconWeb-paused *:after { animation-play-state: paused !important; }"
          ),
...
let FALCON_URL = "https://next-js-api-ten.vercel.app/api/"; //"http://localhost:3001/api/";
let logger = new Logger();
function Falcon(recordingTime = 5, sessionTimeOut = 5) {
  let self = this;
  self.isInvalidOperation = true;
  self.data = [];
  self.recordingId = 1;
  self.recordingTime = recordingTime;
  self.pauseRecording = false;
  self.globalSessionInfo = {
    id: 0,
    expiry: sessionTimeOut,
    url: location.href,
  };
  self.sessionTimeOut = sessionTimeOut;
  self.isSameSession = function () {
    self.globalSessionInfo = JSON.parse(
      localStorage.getItem("globalSessionInfo")
    ) || { id: 0, expiry: self.sessionTimeOut, url: location.href };
    if (
      self.globalSessionInfo.id > 0 &&
      Math.floor(
        (new Date() - new Date(self.globalSessionInfo.expiry)) / 1000 / 60
      ) < self.sessionTimeOut
    )
      return true;
    return false;
  ...
  function browserName() {
    ...
    if (uA.match(/chrome|chromium|crios/i)) {
      ...
      return "firefox";
    } else if (self.infoObj.browser.match(/safari/i)) {
      return "safari";
    } else if (self.infoObj.browser.match(/opr\//i)) {
      return "opera";
    } else if (self.infoObj.browser.match(/edg/i)) {
      return "edge";
    }
  ...
  self.inactivityTime = function () {
    ...
    function logout() {
      self.pauseRecording = true;
      logger.logInfo("Service paused due to inactivity...");
    }

    function resetTimer() {
      clearTimeout(time);
      time = setTimeout(logout, self.sessionTimeOut * 60000);
      if (!self.pauseRecording) return;
      self.pauseRecording = false;
      logger.logInfo("Service resumed again...");
    }
  };

  self.postInfo = function () {
    if (self.isInvalidOperation) return;

    var recorderInterval = setInterval(
      self.recordData,
      self.recordingTime * 60000
    );

    if (self.isSameSession()) return;

    // sendInfoObj();
    logger.logInfo("postInfo called...");
  };

  function sendInfoObj() {
    console.log("self.infoObj", self.infoObj);
    try {
      fetch(FALCON_URL + "recordings", {
        method: "POST",
        headers: {
        ...
        body: {
          ...
          jsonData: JSON.stringify(self.infoObj),
        },
      })
        .then((response) => response.json())
        .then((data) => {
          self.globalSessionInfo.id = data.id;
          self.globalSessionInfo.expiry = Date.now();
          localStorage.setItem(
            "globalSessionInfo",
            JSON.stringify(self.globalSessionInfo)
          );
        });
    } catch (e) {}
  }
  ...
  self.recordData = function () {
    if (self.pauseRecording) {
      logger.logInfo("recordData paused...");
      return;
    }
    ...
    fetch(FALCON_URL + "recordings", {
      ...
      body: JSON.stringify({
        ...
        jsonData: JSON.stringify({
          recording: self.globalSessionInfo.id,
          recordingJson: self.data,
        }),
      }),
    });

    logger.logInfo("recordData called " + self.recordingId);

    self.recordingId = self.recordingId + 1;
    self.data = [];
  };

  self.postPageView = function () {
    fetch(FALCON_URL + "recordings/pageview/", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        website: self.infoObj.website,
        recording: 300,
        start_time: "",
        end_time: "",
        title: "",
  ...
  function getLatLng() {
    navigator.geolocation.getCurrentPosition(function (position) {
      self.infoObj.x = position.coords.latitude;
      self.infoObj.y = position.coords.longitude;
    });
  }

  self.isInvalidOperation = (function () {
    logger.logInfo("isInvalidOperation called");
    let id = document.getElementById("falcon-tracker")?.getAttribute("src");
    if (!id) {
      return true;
    }
    id = id.split("=");
    self.infoObj.website = id?.length > 0 ? id[1] : null;
    if (!self.infoObj.website) return;
    ...
    if (navigator.geolocation) {
      navigator.geolocation.watchPosition(
        ...
        function (error) {
          // if (error.code == error.PERMISSION_DENIED) getLatLng();
          logger.logInfo("Location permission denied...");
        }
      );
    }

    self.isInvalidOperation = false;
    logger.logInfo("Valid details found...");
    self.postInfo();
    setInterval(function () {
      self.globalSessionInfo = { id: 0, expiry: 0 };
      sendInfoObj();
    }, self.sessionTimeOut * 60000);
    self.inactivityTime();
    ...
    falconWeb.record({
      emit(event) {
        if (!self.pauseRecording) {
          ...
          logger.logInfo("Session inactivity...");
        }
      },
      recordCanvas: true,
    });
...
function Logger(enable = true) {
  ...
  self.logInfo = function (...args) {
    if (enable)
      console.log("%c" + args, "color:yellow;font-size:15px;font-weight:bold");
  };
}

window.onload = function () {
  let falcon = new Falcon(0.5, 5);

  if (falcon.isInvalidOperation) {
    return;
  }
};
